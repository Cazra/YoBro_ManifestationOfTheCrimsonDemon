...
public class PortraitAKittehSprite extends DialogPortraitSprite
{
	...
	public PortraitAKittehSprite(double x, double y, String text)
	{
		super(x,y, text);
		numInstances++;
	}
	...
	**/
	...
	public static void loadImages(ImageLoader il)
	{
		...
		imageURL = PortraitAKittehSprite.class.getResource("graphics/AdvKittehPortraits.png");
		Image portraitSheet = tk.getImage(imageURL);
		portraitSheet = ColorFilters.setTransparentColor(portraitSheet, new Color(0xFF00FF));
		
		img = ImageBlitter.crop(portraitSheet,1,1,208,293);
		il.addImage(img);
		focusTable.put(":)",new Point(0,0));
		imageTable.put(":)",img);
		
		img = ImageBlitter.crop(portraitSheet,211,1,208,293);
		il.addImage(img);
		focusTable.put(":D",new Point(0,0));
		imageTable.put(":D",img);
		
		img = ImageBlitter.crop(portraitSheet,421,1,208,293);
		il.addImage(img);
		focusTable.put("TuT",new Point(0,0));
		imageTable.put("TuT",img);
		
		img = ImageBlitter.crop(portraitSheet,631,1,208,293);
		il.addImage(img);
		focusTable.put(">:(",new Point(0,0));
		imageTable.put(">:(",img);
		
		img = ImageBlitter.crop(portraitSheet,1,296,208,293);
		il.addImage(img);
		focusTable.put(":'(",new Point(0,0));
		imageTable.put(":'(",img);
		
		img = ImageBlitter.crop(portraitSheet,211,296,208,293);
		il.addImage(img);
		focusTable.put("D':",new Point(0,0));
		imageTable.put("D':",img);
		
		img = ImageBlitter.crop(portraitSheet,421,296,253,238);
		il.addImage(img);
		focusTable.put(">:O",new Point(0,0));
		imageTable.put(">:O",img);
